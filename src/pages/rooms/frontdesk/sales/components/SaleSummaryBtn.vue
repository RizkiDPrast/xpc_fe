<template>
  <div>
    <q-btn v-bind="$attrs" flat icon="las la-file-alt" label="Summary" color="secondary" @click="model = true" />
    <q-dialog v-model="model" full-width full-height>
      <q-card>
        <dialog-header title="Daily Sales Summary">
          <template #actions>
            <DatePrevNextSalesmanager v-model="date" @loading="loadingFn" @detail-fetched="fetched" />
          </template>
        </dialog-header>
        <my-circular-progress v-if="loading" class="full-width text-center"/>
        <q-card-section v-else style="min-width:300px;min-width:500px" class="row">          
          <q-markup-table class="table col-4" bordered dense flat>
            <tr class="text-primary"><th>Payment</th><th style="text-align:right!important">Total</th></tr>
            <tr>
              <th>Cash</th>
              <td> {{ details.sales.reduce((a,b)=>  a + b.cash - b.change , 0) | money }} </td>
            </tr>
            <tr>
              <th>Transfer</th>
              <td> {{ details.sales.reduce((a,b)=>  a + b.transfer , 0)  | money}} </td>
            </tr>
            <tr>
              <th>Deposit</th>
              <td> {{ details.sales.reduce((a,b)=>  a + b.deposit , 0)  | money}} </td>
            </tr>
            <tr v-for="item in edcTypes" :key="item.id">
              <th>Credit Card {{item.label}}</th>
              <td> {{ details.sales.reduce((a,b)=> b.ccEdcType === item.id ? a + b.creditCard : 0 , 0)  | money}} </td>
            </tr>
            <tr v-for="item in edcTypes" :key="item.id">
          ...
          <q-markup-table class="table col-4" bordered dense flat>
            ...
            <tr v-for="item in details.cashInOuts.filter(x => x.saleId === null && !x.transferToSales && x.cashOut > 0 && x.cashOut > x.cashIn)" :key="item.id">
              <th>{{item.note}}</th>
