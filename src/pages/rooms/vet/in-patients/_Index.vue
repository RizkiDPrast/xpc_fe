<template>
  <q-page class="q-pa-md">
    <q-toolbar>
      <q-toolbar-title>
        Treatment plans
      </q-toolbar-title>
      <q-btn
        v-if="isVet"
        flat
        dense
        icon="las la-plus"
        label="New treatment plan"
        @click="showNfd"
      />
    </q-toolbar>

    <div class="row full-width">
      <div
        class="col-12 col-xs-6 q-pa-md"
        v-for="(val, i) in mergeForms"
        :key="i"
      >
        <q-item clickable :to="`/form/${encodeURIComponent(val.id)}`">
          <q-card class="text-center q-pa-md full-width">
            <q-toolbar>
              <q-toolbar-title>
                {{ val.id }}
              </q-toolbar-title>
              <div>by {{ val.created_by }}</div>
            </q-toolbar>
            <q-card-section class="flex flex-center">
              <q-avatar size="100px" font-size="52px" color="primary">
                <q-img :src="val.imageUri" />
              </q-avatar>
              &nbsp;&nbsp;&nbsp;
              <h6>{{ val.patient }}</h6>
              &nbsp;&nbsp;&nbsp;
              <div>
                <p>start at {{ new Date(val.start_at).toLocaleString() }}</p>
                <p>diagnose: {{ val.diagnose }}</p>
                <p>
                  <i> {{ val.notes }} </i>
                </p>
              </div>
            </q-card-section>
          </q-card>
        </q-item>
      </div>
    </div>
    <new-treatment-plan-dialog v-model="nfd" :form="form" @update:form="save" />
  </q-page>
</template>

<script>
import NewTreatmentPlanDialog from "./components/NewTreatmentPlanDialog";
import forms from "./fake_data/forms";
export default {
  name: "PageIndex",
  components: {
    NewTreatmentPlanDialog
  },
  data() {
    return {
      forms,
      nfd: false,
      form: {
        id: undefined,
        created_by: undefined,
        patient: undefined,
        imageUri: undefined,
        start_at: undefined,
        weight: 0,
        diagnose: undefined,
        notes: undefined,
        treatment_plan: [],
        treatment: {}
      }
    };
  },
  computed: {
    auth() {
      return this.$store.state.auth;
    },
    isVet() {
      return this.auth && this.auth.name.indexOf("Dokter") > -1;
    },
    storedForms() {
      return this.$store.state.forms || [];
    },
    mergeForms() {
      var dt = JSON.parse(JSON.stringify(forms));
      for (var sdt of this.storedForms) {
        dt.push(sdt);
      }
      return dt;
    }
  },
  methods: {
    error(msg) {
      this.$q.notify({
        type: "negative",
        message: msg
      });
    },
    showNfd() {
      this.form = {
        id: "XXX/1230/YYYY",
        created_by: undefined,
        patient: "Cat Meow",
        imageUri:
          "data:image/jpeg;base64,",
        start_at: undefined,
        weight: 0,
        diagnose: "diagnose1",
        notes: "note1",
        treatment_plan: [{ label: undefined, duration: undefined }],
        treatment: {}
      };
      this.nfd = true;
    },
    save(tfrm) {
      this.form = tfrm;
      let frm = JSON.parse(JSON.stringify(this.form));

      if (!frm.id) {
        this.error("Form ID is required");
        return;
      }
      if (!frm.start_at || !new Date(frm.start_at)) {
        this.error("Start at is required");
        return;
      }
      if (!frm.patient) {
        this.error("Please input patient name");
        return;
      }

      if (
        !frm.treatment_plan.length ||
        frm.treatment_plan[0].label == "" ||
        !(frm.treatment_plan[0].duration > 0)
      ) {
        this.error("Please add at least 1 treatment plan");
        return;
      }
      const tasks = ["pagi", "siang", "sore"].map(x => ({
        label: x,
        is_checklist: true,
        value: false,
        done_by: undefined,
        note: undefined
      }));
      // treatment_plan: [{label: '', duration: x}]
      const max = frm.treatment_plan.reduce(
        (a, b) => (a > b.duration ? a : b.duration),
        0
      );
      var t2 = {};
      for (let i = 0; i < max; i++) {
        var d = new Date(frm.start_at);
        d.setDate(d.getDate() + i);
        var k = d.toJSON();
        t2[k] = {};
        for (let t = 0; t < frm.treatment_plan.length; t++) {
          const tr = frm.treatment_plan[t];
          const dur = tr.duration || 0;
          if (dur <= i) {
            break;
          } else {
            console.log(i, k, tr.label);
            t2[k][tr.label] = JSON.parse(JSON.stringify(tasks));
          }
        }
      }
      frm.treatment_plan = frm.treatment_plan.map(x => x.label);
      // treatment_plan: ['']

      frm.treatment = t2;
      frm.created_by = this.auth.name;
      console.log("frm", frm);
      this.$store.commit("forms/push", frm);
      console.log(this.mergeForms);
      // this.$router.push(`/form/${encodeURIComponent(frm.id)}`);

      this.nfd = false;
    }
  }
};
</script>
