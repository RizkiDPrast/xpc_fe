          ...
          <q-card >
            ...
            <q-list bordered style="min-height:50px;">
              <q-item v-for="model in item.data" :key="model.id">
                ...
                <q-item-section>
                  <q-item-label>
                    {{ getAnimalTypeName(model.patient.animalTypeId) }} - {{ model.patient.name }}         
                  </q-item-label>
                  <q-item-label caption>
...
export default {
  ...
  computed: {
    dataModel(){
      ...
      let today = this.data.filter(x => this.$util.getDateDiff(tday, x.createdAt, 'days') === 0);
      let yesterday = this.data.filter(x => this.$util.getDateDiff(tday, x.createdAt, 'days') === 1);
      let others =this.data.filter(x => this.$util.getDateDiff(tday, x.createdAt, 'days') > 1)
      return[
        {label:'Today', caption: `${today.length} patient${today.length > 1 ? 's' : ''}`, data: today},
        {label:'Yesterday', caption: `${yesterday.length} patient${yesterday.length > 1 ? 's' : ''}`, data: yesterday},
        {label:'Older', caption: `${others.length} patient${others.length > 1 ? 's' : ''}`, data: others},
        ]
    }
  },
  mounted() {
