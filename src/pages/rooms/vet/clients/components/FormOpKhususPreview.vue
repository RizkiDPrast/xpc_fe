    ...
    content() {
      ...
      let body = `
          ...
          <p style="box-sizing:border-box;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif,'Apple Color Emoji','Segoe UI Emoji','Segoe UI Symbol';font-size:16px;line-height:1.5em;margin-top:0;text-align:left">
            ...
            <p>
                Memahami informasi yang disampaikan oleh dokter hewan xing pet care and clinic 
                bahwa ada resiko tertular penyakit jika ${
                  model.animalType
                } saya yang bernama <b> ${model.patientName}</b> 
                dioperasi steril dengan status : <b> ${nullable(
                  model.status
                )} </b>. Salah satu contohnya penyakit panleukopenia virus. 
                Namun karena pertimbanagan saya yaitu ${nullable(
                  model.pertimbangan
                )} maka saya memutuskan untuk 
                meminta ${
                  model.animalType
                } saya untuk tetap disteril dengan status tersebut.
                ${
                  model.animalType
                } tersebut setelah disteril akan dirawat di ${nullable(
        model.dirawatDi
      )} 
                dan diberikan vaksin ${nullable(
                  model.vaksinSetelahOp
                )} setelah kondisi pulih.
            </p>
            <p>
                Adapun resiko yang diterima adalah sepenuhnya tanggung jawab saya 
                dan saya mengizinkan ${nullable(model.animalType)} bernama ${
        model.patientName
      } untuk di treatment lebih lanjut sampai sembuh 
                jika memang terpapar penyakit. 
                Saya tidak akan menuntut apapun karena steril dengan status vaksin tersebut adalah permintaan saya.
            </p>
        `;
