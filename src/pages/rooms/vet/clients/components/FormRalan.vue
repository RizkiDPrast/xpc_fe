<template>
  <q-form @submit="onSubmit" class="">
    <div class="row q-col-gutter-sm">
      ...
      <q-checkbox
        class="col-sm-12 col-md-12 q-my-sm q-py-xs"
        v-model="model.setujuDatang"
        name="setujuDatang"
        label="Setuju Datang"
        v-validate="'requiredTrueFalse'"
        :error="errors.has('setujuDatang')"
        :error-message="errors.first('setujuDatang')"
        :readonly="loading"
        dense
        outlined
        autocomplete="off"
      ...
      <q-checkbox
        class="col-sm-12 col-md-12 q-my-sm q-py-xs"
        v-model="model.setujuTest"
        name="setujuTest"
        label="Setuju Test"
        v-validate="'requiredTrueFalse'"
        :error="errors.has('setujuTest')"
        :error-message="errors.first('setujuTest')"
        :readonly="loading"
        dense
        outlined
        autocomplete="off"
      ...
      <q-input
        class="col-sm-12 col-md-12 q-my-sm q-py-xs"
        v-model="model.contohTest"
        type="text"
        name="contohTest"
        label="Contoh Test"
        v-validate="'required'"
        :error="errors.has('contohTest')"
        :error-message="errors.first('contohTest')"
        :readonly="loading"
        dense
        outlined
        autocomplete="off"
...
export default {
  ...
  methods: {
    async onSubmit() {
      ...
      if (
        typeof this.model.setujuDatang === "undefined" ||
        this.model.setujuDatang === null
      ) {
        this.$toastr.error("Please check / uncheck all checkboxes");
        return;
      }
      if (
        typeof this.model.setujuTest === "undefined" ||
        this.model.setujuTest === null
      ) {
        this.$toastr.error("Please check / uncheck all checkboxes");
        return;
      }
